...
class StxScriptTransformer(lark.Transformer):
    def start(self, statements):
        return "\n".join(str(statement) for statement in statements if statement)

    def variable_declaration(self, items):
        # Handle: "let" IDENTIFIER [":" type] "=" expression ";"
        if len(items) == 3:  # let x = value;
            name, value = items[0], items[1]
            return f"(define-data-var {name} {self.infer_type(str(value))} {value})"
        else:  # let x: type = value;
    ...
    def function_declaration(self, items):
        ...
        # Check for decorators at the beginning
        while offset < len(items) and hasattr(items[offset], 'data') and items[offset].data == 'decorator':
            decorators.append(str(items[offset]))
            offset += 1

        name = items[offset]
        params = items[offset + 1] if offset + 1 < len(items) and hasattr(items[offset + 1], 'data') and items[offset + 1].data == 'parameter_list' else []
        body = items[-1]  # Last item is always the block

        is_public = any('@public' in str(d) for d in decorators)
        ...
        if params:
            params_str = " ".join(f"({p.children[0]} {p.children[1]})" for p in params.children)
        else:
            params_str = ""
